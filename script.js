...
// NYTT: Gräver rekursivt igenom en nods barn och plockar ut text/ackord/länkar.
  ...
  async fetchSongsFromCloud() {
    ...
    this.cloudListener = onSnapshot(songsRef, (snapshot) => {
      ...
      // Räkna ihop alla ändringar
      let totalChanges = 0;
      snapshot.docChanges().forEach(() => totalChanges++);
      const isFreshLoad = this.isBandSwitching && totalChanges > 0;
      ...
      if (needsRefresh || isFreshLoad) {
        if (isFreshLoad) this.isBandSwitching = false;
        this.loadProject(this.titleInput.value || localOrder[0]);
        if (this.btnMainEditToggle) {
...
list.innerHTML = '<option value="">Load song...</option>';
    ...
    // --- 1. FLYTTA UT DESSA VARIABLER OVANFÖR LOOPEN ---
    const insertLine = document.createElement("div");
    insertLine.className = "drag-insert-line";
    let insertBeforeIndex = null; // null = längst ner i listan

    // --- 2. LÄGG TILL LYSSNAREN PÅ DROPDOWNEN EN ENDA GÅNG ---
    dropdown.addEventListener("dragover", (e) => {
        e.preventDefault();
        // Om musen är under alla låtar — placera linjen allra sist
        ...
        if (lastItem && e.clientY > lastItem.getBoundingClientRect().bottom) {
            dropdown.appendChild(insertLine);
            insertBeforeIndex = null; 
        }
    });

    // --- 3. NU STARTAR LOOPEN FÖR VARJE LÅT SOM VANLIGT ---
    order.forEach((name, index) => {
      ...
      // --- 4. DE RENODLADE EVENT-LYSSNARNA FÖR VARJE RAD ---
      ...
      handle.addEventListener("dragend", () => {
          ...
          document.body.classList.remove("is-dragging");
          insertLine.remove(); // Städar bort linjen om man avbryter
          insertBeforeIndex = null;
      });
      ...
      item.addEventListener("dragover", (e) => {
          ...
          // Eftersom 'insertLine' är unik flyttar webbläsaren den automatiskt hit!
          if (insertBefore) {
              dropdown.insertBefore(insertLine, item);
              insertBeforeIndex = index;
          } else {
              dropdown.insertBefore(insertLine, item.nextSibling);
      ...
      item.addEventListener("drop", (e) => {
          e.preventDefault();
          insertLine.remove(); // Ta bort linjen direkt vid släpp

          const fromIndex = parseInt(e.dataTransfer.getData("text/plain"));
          let toIndex = insertBeforeIndex ?? order.length;

          // Justera indexet om vi flyttar en låt nedåt i listan
...
async checkUserBand(uid) {
  ...
  async createBand() {
    ...
    try {
      // 1. Skapa bandet i databasen
      await setDoc(
        doc(db, "bands", bandCode),
        { name: bandName, members: [uid] },
        { merge: true }
      );
  ...
  async leaveBand() {
    ...
    this.myBands = (this.myBands || []).filter(
      (b) => b.id !== this.currentBandId
    );
    ...
    try {
      ...
      this.showCustomAlert(
        nextBandId
          ? `Switched to ${nextBandName}`
          : "You are playing solo again!"
      );
    } catch (e) {
      console.error(e);
    }
  ...
  async switchBand(bandId, bandName) {
    ...
    try {
      ...
      const displayName = safeBandName
        ? `"${safeBandName}"`
        : "Personal Library (Solo)";

      await new Promise((resolve) => {
        const dialog = document.getElementById("custom-alert");
        document.getElementById(
          "custom-alert-message"
        ).textContent = `You are now in ${displayName}. Press OK to load the song library.`;
        dialog.classList.add("visible");
  ...
  updateBandUI() {
    ...
    let bandsHtml = `<div style="display: flex; flex-direction: column; gap: 8px; margin-bottom: 20px; max-height: 250px; overflow-y: auto; padding-right: 5px;">`;

    // --- 1. SOLO LÄGET ---
    const isSoloActive = !this.currentBandId;
    bandsHtml += `
      <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.2); padding: 8px 12px; border-radius: 8px; border: ${
        isSoloActive ? "1px solid var(--primary)" : "1px solid transparent"
      };">
        <div style="display: flex; flex-direction: column; text-align: left;">
          <strong style="color: ${
            isSoloActive ? "var(--primary)" : "inherit"
          }">👤 Personal Library (Solo)</strong>
        </div>
        ${
          isSoloActive
    ...
    // --- 2. LOOPA IGENOM BANDEN ---
    const bandsList = this.myBands || [];
    bandsList.forEach((b) => {
      const isActive = b.id === this.currentBandId;
      bandsHtml += `
        <div style="display: flex; justify-content: space-between; align-items: center; background: rgba(0,0,0,0.2); padding: 8px 12px; border-radius: 8px; border: ${
          isActive ? "1px solid var(--primary)" : "1px solid transparent"
        };">
          <div style="display: flex; flex-direction: column; text-align: left;">
            <strong style="color: ${
              isActive ? "var(--primary)" : "inherit"
            }">🎸 ${b.name}</strong>
            <span style="font-size: 0.7em; opacity: 0.6;">Invite Code: ${
              b.id
            }</span>
          </div>
          ${
            isActive
              ? `<span style="font-size: 0.8em; opacity: 0.8; font-weight: bold; color: var(--primary);">ACTIVE</span>`
              : `<button class="btn-secondary-style" style="width: auto; padding: 4px 10px; font-size: 0.8em; margin: 0;" onclick="app.switchBand('${
                  b.id
                }', '${b.name.replace(/'/g, "\\'")}')">Switch</button>`
          }
        </div>
      `;
    });

    // --- STÄNG WRAPPERN (En enda gång!) ---
    bandsHtml += `</div>`;
    ...
    modalBox.innerHTML = `
      ...
      <div class="sidebar-controls vertical" style="gap: 0.8em; min-width: 250px; margin-bottom: 20px;">
          ...
          ${
            this.currentBandId
              ? `<button id="btn-band-leave" class="btn-danger" style="margin-top: 10px;">Leave Current Band</button>`
              : ``
          }
      </div>
